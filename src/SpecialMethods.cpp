...
BitArray* SpecialMethods::getSubArray(const BitArray* const bitarray, size_t first_index, size_t last_index) {
...
void SpecialMethods::printHex(const BitArray& bitarray) {
	size_t size = bitarray.getSize();
	size_t number_to_print = 0;
	size_t bytes = size / BYTE;
	size_t bits = size % BYTE;
	if (bytes) {
