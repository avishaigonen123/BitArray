...
size_t BitArray::fromBinaryStr(const char* i_binaryStr, size_t binaryStrLen) {
	...
	size_t* ptr_storage = this->m_storage;
	size_t current = NULL, i = 0;

	if (binaryStrLen > this->m_size_bits) // I assume binaryStrLen is without NULL
		this->setAt(binaryStrLen, false); // we'll call scale with the right capacity
